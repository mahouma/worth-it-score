...
function getPeriodLabel(period) {
  ...
  const ot = period - 4;
  return { label: ot === 1 ? "Overtime" : `${ot}x Overtime`, ot: true };
}
    ...
    <main className={dmMono.className} style={{ minHeight: "100vh", background: "#080808", color: "#f0ede8" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 640, margin: "0 auto", padding: "72px 24px 96px" }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <p style={{ fontSize: 9, letterSpacing: "0.3em", textTransform: "uppercase", color: "#e85a1a", marginBottom: 8 }}>NBA · Game Detail</p>
          {formattedDate && <p style={{ fontSize: 12, color: "#5a5a5a" }}>{formattedDate}</p>}
        </div>

        {worthIt !== null && (
          <div style={{ background: "#111111", border: "1px solid #1e1e1e", padding: "32px 32px 28px", marginBottom: 2, display: "flex", alignItems: "center", gap: 28 }}>
            <span
              className={bebasNeue.className}
              style={{ fontSize: 120, lineHeight: 1, color: scoreColor, letterSpacing: "-0.01em", flexShrink: 0 }}
            >
              {worthIt}
            </span>
            <div>
              <p style={{ fontSize: 9, letterSpacing: "0.25em", textTransform: "uppercase", color: "#2e2e2e", marginBottom: 6 }}>Worth-It Score</p>
              <p style={{ fontSize: 28, fontWeight: 500, color: scoreColor, lineHeight: 1.1, marginBottom: 12 }}>{caption}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <ScoreLine label="Closeness" value={Math.abs(hs - vs) <= 5 ? "Tight" : Math.abs(hs - vs) <= 10 ? "Competitive" : Math.abs(hs - vs) <= 20 ? "One-sided" : "Blowout"} />
                <ScoreLine label="Overtime" value={period > 4 ? `Yes - ${period - 4} OT` : "No"} />
                <ScoreLine label="Total scoring" value={`${hs + vs} pts`} />
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ background: "#111111", border: "1px solid #1e1e1e", padding: "40px 32px", marginBottom: 2 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 24, marginBottom: 40 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ fontSize: 9, letterSpacing: "0.25em", textTransform: "uppercase", color: visitorWon ? "#e85a1a" : "#2a2a2a", marginBottom: 8 }}>{visitor.abbreviation}</p>
              <p style={{ fontSize: 14, color: visitorWon ? "#f0ede8" : "#5a5a5a", fontWeight: visitorWon ? 500 : 300, lineHeight: 1.3 }}>{visitor.full_name}</p>
              {hasScore && (
                <p className={bebasNeue.className} style={{ fontSize: 80, lineHeight: 1, color: visitorWon ? "#f0ede8" : "#2a2a2a", marginTop: 16 }}>{vs}</p>
              )}
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0, textAlign: "right" }}>
              ...
              <p style={{ fontSize: 9, letterSpacing: "0.25em", textTransform: "uppercase", color: homeWon ? "#e85a1a" : "#2a2a2a", marginBottom: 8 }}>{home.abbreviation}</p>
              <p style={{ fontSize: 14, color: homeWon ? "#f0ede8" : "#5a5a5a", fontWeight: homeWon ? 500 : 300, lineHeight: 1.3 }}>{home.full_name}</p>
              {hasScore && (
                <p className={bebasNeue.className} style={{ fontSize: 80, lineHeight: 1, color: homeWon ? "#f0ede8" : "#2a2a2a", marginTop: 16 }}>{hs}</p>
              )}
            </div>
          </div>
          ...
          <div style={{ borderTop: "1px solid #161616", paddingTop: 24, display: "flex", gap: 40, flexWrap: "wrap" }}>
            ...
            {hasScore && (
              <div>
                ...
                <p style={{ fontSize: 12, color: "#5a5a5a" }}>{homeWon ? home.full_name : visitorWon ? visitor.full_name : "Tie"}</p>
              </div>
            )}
          </div>
        </div>
